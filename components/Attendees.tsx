import { ActionIcon, ScrollArea, Switch, Table, TextInput, useMantineTheme } from "@mantine/core";
import { IconCheck, IconClock, IconPlus, IconX } from "@tabler/icons";
import { useState } from "react";

export default function Attendees({ poytakirja, setPoytakirja }: { poytakirja: Poytakirja, setPoytakirja: (value: Poytakirja) => void }) {
    ...
    <tr key={member.name}>
      ...
      <td>
        <Switch
          ...
          onChange={(event) => {member.absent = !event.currentTarget.checked; setPoytakirja(pkc)} }
          color="teal"
          size="md"
      ...
      <td>
        <TextInput placeholder="15:18" pattern="\d\d:\d\d" maxLength={5} value={member.joinedLate} onClick={(event) => {
          ...
          member.joinedLate = `${new Date().getHours().toString().padStart(2, '0')}:${new Date().getMinutes().toString().padStart(2, '0')}`;
          setPoytakirja(pkc);
      ...
      <td>
        <TextInput placeholder="16:12" pattern="\d\d:\d\d" maxLength={5} value={member.left} onClick={(event) => {
          ...
          member.left = `${new Date().getHours().toString().padStart(2, '0')}:${new Date().getMinutes().toString().padStart(2, '0')}`;
          setPoytakirja(pkc);
        }} ><IconClock size="xs" /></ActionIcon>}/>
      </td>
    </tr>
    ...
    <tr key={member.name}>
      ...
      <td>
        <ActionIcon color="red" onClick={() => {
          poytakirja.other.splice(poytakirja.other.map(i => i.name).indexOf(member.name), 1);
          setPoytakirja(pkc);
        }}>
          <IconX size={18} />
        </ActionIcon>
