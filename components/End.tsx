import { ActionIcon, MultiSelect, Select, Space, TextInput } from "@mantine/core";
...
export function End({
  ...
  return (
    <>
      ...
      <TextInput
        value={poytakirja.nextMeeting}
        onChange={(event) => {
          pkc.nextMeeting = event.currentTarget.value as any;
          setPoytakirja(pkc);
        }}
        label="Seuraava kokous on:"
        sx={{
      ...
      <TextInput
        ...
        label="Puheenjohtaja päätti kokouksen klo"
        rightSection={
          <ActionIcon
            onClick={() => {
              pkc.closeTime = `${new Date()
                .getHours()
                .toString()
